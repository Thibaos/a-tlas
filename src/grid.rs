...
/// The lattice's half-extent in voxels, derived from the Region budget:
/// [`REGION_HALF_EXTENT`] × [`REGION_EDGE`] = 2048. Global voxel coords
/// must lie in [-2048, 2048) on every axis.
pub const LATTICE_HALF_EXTENT: i32 = REGION_HALF_EXTENT * REGION_EDGE;

/// Floor-divides `global` by `edge` per axis: the index of the
/// origin-aligned cell of size `edge` containing `global`. Floor (not
/// truncation), so negative coords land in the correct cell (e.g. -8 / 8 →
/// -1, not 0).
pub fn grid_index(global: IVec3, edge: i32) -> IVec3 {
    global.div_euclid(IVec3::splat(edge))
}

/// The min corner of the origin-aligned cell of size `edge` containing
/// `global` (a multiple of `edge`).
pub fn grid_origin(global: IVec3, edge: i32) -> IVec3 {
    grid_index(global, edge) * edge
}

/// Whether `global` lies inside the renderer lattice on every axis (half-open
/// [-[`LATTICE_HALF_EXTENT`], [`LATTICE_HALF_EXTENT`])).
...
/// [-[`REGION_HALF_EXTENT`], [`REGION_HALF_EXTENT`]) → voxel ±2048/axis).
/// The single source of truth for the extent: [`region_id`] and the
/// world→renderer boundary check both share it, so the bounds cannot drift.
...
/// Panics iff `region_index` is outside the v1 lattice. Unconditional (not a
/// `debug_assert`): in release the 4-bit encoding in [`region_id`] would
/// silently alias out-of-lattice indices and index past the fixed Region
/// tables, corrupting (not erroring). An over-lattice model must fail loudly
/// at the world→renderer boundary.
...
/// The 12-bit Region id (rides the TLAS instance custom index and indexes the
/// Region table): 4 bits per axis over the signed Region lattice index, biased
/// by +[`REGION_HALF_EXTENT`] so all-16 values per axis fit the 4 bits.
...
/// The Region lattice index of a global coord (floor division, so negative
/// coords land in the correct origin-aligned Region).
pub fn region_index_of(global_coords: IVec3) -> IVec3 {
    grid_index(global_coords, REGION_EDGE)
}
...
mod tests {
    ...
    fn extent_derives_from_region_budget() {
        ...
        assert_eq!(LATTICE_HALF_EXTENT, REGION_HALF_EXTENT * REGION_EDGE);
