...
The path traced lighting effort's shading half: the recursive path loop that
turns one primary ray per pixel into the diffuse+specular radiance pair
...
**Diffuse and specular are separated at the primary hit by probabilistic
lobe selection**: a coin flip (p = `LOBE_P` = 0.5, inside NRD's [1/4, 3/4]
clamp for the AREA_3X3 hit-distance reconstruction mode) picks the lobe
the first Bounce samples; the whole path's radiance goes to the selected
...
**Sampling**: cosine-weighted hemisphere for the Lambertian lobe (bounce
weight = `(1-metallic) × albedo`); GGX VNDF (Heitz 2018) for the specular
lobe with Cook-Torrance BRDF eval (separable Smith G2, Schlick Fresnel with
`F0 = mix(0.04, albedo, metallic)`, alpha = roughness² floored at
`ALPHA_MIN`); the estimator is computed explicitly as `f_s · (N·L) / pdf_L`.

**De-modulation is written now (Q4)**: the diffuse channel is divided by
the primary albedo (eps-guarded, `ALBEDO_EPS = 1e-3`) so the Denoise pass
filters pure light; the Composite re-modulates by the same
...
**Termination**: Russian roulette on the throughput's max component with a
`RR_FLOOR` of 0.05 (continuation weight divides back out); `MAX_BOUNCES` 4
is the safety cap. The Background is black in this slice (the miss shader's
