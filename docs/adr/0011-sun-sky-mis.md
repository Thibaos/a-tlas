...
The path traced lighting effort's analytic lights: the Sun and Procedural
sky, sampled by next-event estimation with multiple importance sampling
against the BSDF, and the Procedural sky as the new Background. Decided
with the owner (ticket 06) and landed as the miss shader's output and the
NEE block in the path loop (`shaders/region/production.rgen`).
...
comparison). The shadow ray reuses the DDA hit group from the normal-nudge
origin and consumes no RNG draws.
...
the full BRDF. The MIS balance heuristic covers the strategies that can
produce a direction:
...
(sky only) the env's (φ, μ) draws, then (deeper) the 50/50 lobe pick, the
two direction draws, the Russian-roulette draw; the shadow trace consumes
none. The `vndf_pdf` helper was extracted from `sample_vndf` (identical
...
**The byte-exact validator is untouched**: it keeps the black miss shader
(the capture pipeline selects a capture-specific module; the production
pipeline selects the sky miss). Its Reference comparison stays against the
constant background. The Scene buffer (sun direction, `E_sun`, sky knots,
disk cos-radius and radiance) is a production-read storage buffer created
in the store and updated every frame from the render context (tunable);
the capture stages declare the binding but never dereference it, so the
validator's 12 worlds still pass byte-exact (verified).

Defaults (tunable, not locked): `sun_dir = normalize(0.45, 0.8, 0.35)`
(~52° elevation), `E_sun = 16`, knots ground 0.15 / horizon 0.6 / zenith
1.2, `θ_disk = 0.5°`.
